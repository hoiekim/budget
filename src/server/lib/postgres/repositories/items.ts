...
import {
  MaskedUser, ItemModel, itemsTable, accountsTable, transactionsTable, investmentTransactionsTable,
  splitTransactionsTable, snapshotsTable, holdingsTable,
  ITEM_ID, USER_ID, INSTITUTION_ID, ACCOUNT_ID,
} from "../models";
...
export const getItemsByInstitution = async (user: MaskedUser, institution_id: string): Promise<JSONItem[]> => {
  const models = await itemsTable.query({ [USER_ID]: user.user_id, [INSTITUTION_ID]: institution_id });
  return models.map(m => m.toJSON());
};

export const getUserItem = async (item_id: string): Promise<{ user: MaskedUser; item: JSONItem } | null> => {
...
export const upsertItems = async (user: MaskedUser, items: PartialItem[], upsert: boolean = true): Promise<UpsertResult[]> => {
...
export const deleteItems = async (user: MaskedUser, item_ids: string[]): Promise<{ deleted: number }> => {
