...
// reaches SQL. Without the guard, a non-UUID `budget_id` reaches the `WHERE`
// clause and `roll_over_start_date: "hello"` reaches a DATE column, Postgres
// raises `22P02` / `22007` at the write, the route rethrows, and
// `Route.execute` answers 500 **and** calls `sendAlarm`. A client type error
// must not page, and must not spend a slot of `alarm.ts`'s global per-window
// send ceiling that a real fault needs.
