// Per-test-bundle variant of api_keys.test.ts. The source under test
// (api_keys.ts) is built into .test-bundles/ with `pg` kept external;
// this test mocks `pg` at the leaf level and dynamic-imports the bundle.
// Because the bundle path is unique to this test, the dynamic import
// captures THIS test's mocks at load time, independent of other tests
// running in the same `bun test` process.
...
const { generateApiKey, hashApiKey, createApiKey, listApiKeys, revokeApiKey, verifyApiKey } =
  await import(
    "../../../.test-bundles/src__server__lib__postgres__repositories__api_keys.bundle.js"
  );
