// Per-test-bundle variant of accounts.test.ts. Bundles accounts.ts with
// pg/bcrypt/plaid kept external; mocks `pg` at the leaf level so each
// invocation of `new Pool()` inside the bundle returns FakePool.
...
const { getAccounts, getAccount, searchAccounts, searchAccountsById, upsertAccounts } =
  await import(
    "../../../.test-bundles/src__server__lib__postgres__repositories__accounts.bundle.js"
  );
